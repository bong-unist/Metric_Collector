...
#		if self.analyzer_type != 1 and sql_data1["data_len"] != sql_data2["data_len"]: return False
		...
		while True:
			...
			for idx in range(sidx, eidx):
				...
				while idx1 < len1 and idx2 < len2:
					match = False
					diff_data_len = abs(sql_data1[idx1]["data_len"] - sql_data2[idx2]["data_len"])
					if diff_data_len <= self.alpha:
						match = self.__store_process__(fid, server1, server2, sql_data1[idx1], sql_data2[idx2])
						if match == True:
							metadata_key_ = (sql_data2[idx2]["node_id"], fid)
							prev_data_len[metadata_key_] = sql_data2[idx2]["data_len"]

					if sql_data1[idx1]["data_len"] > sql_data2[idx2]["data_len"]: 
						idx2 += 1
						if idx2 < len2 and sql_data1[idx1]["data_len"] <= sql_data2[idx2]["data_len"] and match == False:
							pass
							self.__store_process__(fid, server1, server2, sql_data1[idx1], sql_data2[idx2 - 1])
					elif sql_data1[idx1]["data_len"] < sql_data2[idx2]["data_len"]: 
						idx1 += 1
						if idx1 < len1 and sql_data1[idx1]["data_len"] >= sql_data2[idx2]["data_len"] and match == False:
							pass
							self.__store_process__(fid, server1, server2, sql_data1[idx1 - 1], sql_data2[idx2])
					else:
						idx1 += 1; idx2 += 1
			...
			for idx in range(sidx, eidx):
				...
				for server in server_list:
					...
					while idx1 < len1 and idx2 < len2:
						...
						diff_data_len = abs(sql_data1[idx1]["data_len"] - sql_data2[idx2]["data_len"])
						if diff_data_len <= self.alpha:
							match = self.__store_process__(fid, server_host, server_vm, sql_data1[idx1], sql_data2[idx2])
							if match == True:
								metadata_key_ = (sql_data2[idx2]["node_id"], fid)
								prev_data_len[metadata_key_] = sql_data2[idx2]["data_len"]
							...
							if sql_data1[idx1]["data_len"] > sql_data2[idx2]["data_len"]:
								...
								if idx2 < len2 and sql_data1[idx1]["data_len"] <= sql_data2[idx2]["data_len"] and match == False:
									pass
									self.__store_process__(fid, server_host, server_vm, sql_data1[idx1], sql_data2[idx2 - 1])
							elif sql_data1[idx1]["data_len"] < sql_data2[idx2]["data_len"]:
								idx1 += 1
								if idx1 < len1 and sql_data1[idx1]["data_len"] >= sql_data2[idx2]["data_len"] and match == False:
									pass
									self.__store_process__(fid, server_host, server_vm, sql_data1[idx1 - 1], sql_data2[idx2])
								else:
									idx1 += 1; idx2 += 1
		...
		while True:
			...
			for idx in range(sidx, eidx):
				...
				while idx1 < len1 and idx2 < len2:
					...
					diff_data_len = abs(sql_data1[idx1]["data_len"] - sql_data2[idx2]["data_len"])
					if diff_data_len <= self.alpha:
						match = self.__store_process__(fid, server1, server2, sql_data1[idx1], sql_data2[idx2], False)
						if match == True:
							metadata_key_ = (sql_data2[idx2]["node_id"], fid)
							prev_data_len[metadata_key_] = sql_data2[idx2]["data_len"]
					...
					if sql_data1[idx1]["data_len"] > sql_data2[idx2]["data_len"]:
						...
						if idx2 < len2 and sql_data1[idx1]["data_len"] <= sql_data2[idx2]["data_len"] and match == False:
							pass
							self.__store_process__(fid, server1, server2, sql_data1[idx1], sql_data2[idx2 - 1], False)
					elif sql_data1[idx1]["data_len"] < sql_data2[idx2]["data_len"]:
						idx1 += 1
						if idx1 < len1 and sql_data1[idx1]["data_len"] >= sql_data2[idx2]["data_len"] and match == False:
							pass
							self.__store_process__(fid, server1, server2, sql_data1[idx1 - 1], sql_data2[idx2], False)
					else:
						if sql_data1[idx1]["evt_type"] < sql_data2[idx2]["evt_type"]: idx1 += 1
						else: idx2 += 1
			...
			for fid in flow_id:
				for idx in range(sidx, eidx):
					...
					while evt2 < len(cache_evt_type[metadata_key]):
						...
						idx1 = 0; idx2 = 0
						len1 = len(sql_data1); len2 = len(sql_data2)
						...
						while idx1 < len1 and idx2 < len2:
							...
							plus1 = 0
							plus2 = 0
							if sql_data1[idx1]["evt_type"] == 1: plus1 = 24312
							if sql_data2[idx2]["evt_type"] == 2: plus2 = 24312
							diff_data_len = abs(sql_data1[idx1]["data_len"] + plus1 - sql_data2[idx2]["data_len"] + plus2)
							if diff_data_len < self.alpha:
								match = self.__store_process__(fid, server, server, sql_data1[idx1], sql_data2[idx2])
							
							if sql_data1[idx1]["data_len"] + plus1 > sql_data2[idx2]["data_len"] + plus2:
								idx2 += 1
								if idx2 < len2 and sql_data1[idx1]["data_len"] + plus1 <= sql_data2[idx2]["data_len"] + plus2 and match == False:
									pass
									self.__store_process__(fid, server, server, sql_data1[idx1], sql_data2[idx2])
							elif sql_data1[idx1]["data_len"] + plus1 < sql_data2[idx2]["data_len"] + plus2:
								idx1 += 1
								if idx1 < len1 and sql_data1[idx1]["data_len"] + plus1 >= sql_data2[idx2]["data_len"] + plus2 and match == False:
									pass
									self.__store_process__(fid, server, server, sql_data1[idx1], sql_data2[idx2])
							else:
								idx1 += 1; idx2 += 1
