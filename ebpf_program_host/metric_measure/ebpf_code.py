...
class ebpfCode:
	def __init__(self):
		self.func_name = ["virtio_dev_tx_split", "virtio_dev_tx_packed", "mlx5_rx_burst_vec", "virtio_dev_rx_split", "virtio_dev_rx_packed", "mlx5_tx_burst_none_empw"]
	...
	def __set_variable__(self):
		...
		self.pkt_cnt_pos["mlx5_rx_burst_vec"] = "PT_REGS_RC(ctx)"
	...
	def __set_common_func__(self):
		return """
			...
			static inline u64 get_data_len(struct rte_ipv4_hdr iphdr, struct rte_tcp_hdr tcphdr, struct flow_info flow_info, u32 cur_seq, u32 prev_seq) {
				u64 data_len;
				u64 data_len_ = bpf_ntohs(iphdr.total_length) - ((tcphdr.data_off >> 4) << 2) - (iphdr.ihl << 2);

				if (prev_seq) { 
					if (prev_seq > cur_seq) {
						data_len = (UINT_MAX - (u64) prev_seq) + (u64) cur_seq;
					}
					else data_len = (u64) cur_seq - (u64) prev_seq;
					
					if (!data_len_ && data_len == 1) data_len = data_len - 1;
				}
				else {
					data_len = data_len_;
				}
			...
			static inline u64 set_data_len(struct flow_info flow_info, u64 *start_data_len, u32 data_len) {
			...
			static inline void event_occur(struct flow_info flow_info, u64 start_data_len, u64 data_len, u8 evt_type) {
				...
				if (data) {
					...
					data->start_data_len = start_data_len;
	...
	def __set_body_func__(self):
		return """
			int #function_name#(struct pt_regs *ctx) {
				...
				u64 start_data_len = 0, data_len, zero = 0;
				u32 prev_seq;
				s16 i;
				...
				for (i = 0; i < PACKET_PARSE; i++) {
					...
					data_len = set_data_len(flow_info, &start_data_len, get_data_len(iphdr, tcphdr, flow_info, seq, prev_seq));
					
					if (is_event_occur(flow_info, data_len)) {
						event_occur(flow_info, start_data_len, data_len, #evt_type#);
